...
if __name__ == '__main__':
    ...
    path = join("data", subject, session)
    model_name = 'EEGNet'

    for classes in [['MM', 'MI'], ['MI', 'Rest']]:
        output_path = join("output", subject, session)

        train_loader, val_loader, test_loader, full_train_loader = dataLoader.load(path, batch_size,
        ...
        model_path = join(output_path, "models", model_name, version, f"{model_name}_{version}_epoch_{best_epoch}.pt")
        ...
        log_path = join(output_path, "logs", model_name, full_train_version + "_retrain")

        model_path = join(output_path, "models", model_name, full_train_version + "_retrain", f"{model_name}_{full_train_version}_epoch_{full_train_epochs}.pt")

        test_acc, avg_test_loss, avg_diff = test(model_name, test_loader, model_path)
        ...
        if classes[0] == 'MM':
            rename(model_path, join(output_path, "mimm.pt"))
        if classes[0] == 'MI':
            rename(model_path, join(output_path, "restmi.pt"))
